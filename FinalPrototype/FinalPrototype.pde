...
void draw() {
  ...
  for (int i=trialIndex; i<trialCount; i++) // reduces over time
  {
    ...
    strokeWeight(3f);
    if (trialIndex==i)
      stroke(255, 0, 0, 180); //set color to semi translucent  
    else if (trialIndex + 1 == i)
      stroke(255, 150, 150, 150); //set color to semi translucent  
    else
  ...
  noStroke();
  circle(rotHandleX, rotHandleY, 20);
  ...
  float modRotation = target.rotation;
  if (modRotation >= 45 && modRotation < 135) {
    modRotation += 90;
  }
  else if (modRotation >= 225 && modRotation < 315) {
    modRotation -= 90;
  }
  else if (modRotation > 315 || modRotation < 45) {
    modRotation += 180;
  }
  modRotation +=180;
  ...
  rectMode(CENTER);
  rect(0, target.z/2 + handleExtension, (target.z/2 + handleExtension)*radians(10), inchToPix(.1f));
